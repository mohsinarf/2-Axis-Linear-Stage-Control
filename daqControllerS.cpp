    ...
    if (!pstCard->bSetError)
    {
        ...
        */
        if (success)
        {
            ...
            success = bSpcMSetupTrigExternal (pstCard, SPC_TM_HIGH, false); //false sets the trigger to 1M-OHM impedence
...
void daqController::configFetchThread()
{
  ...
  fetcherObj->setPars(this->pstCard,daqInfoPtr->ScanPoints,binaryWfmBuff,OsciModeEn,dwDataBufLen);
...
void fetcher::fetch()
{
    ...
    spcm_dwSetParam_i32(pstCard_f->hDrv, SPC_TIMEOUT, 10000);
