...
void dataProcessor::newWfmCopyToArray_slot(short *wfmPtr, int waveformNumber)
{
    ...
    int chMap[4] = {0,1,2,3}; //for 59xx card
    ...
    if (waveformNumber==0) //memory sanity check once at start
    {
        ...
        if (applyRealTimeFilter)
        {
            if (subbandDecomp)
            {
                startParallelProcThread(1,1,dataBundle[3],dataBundle[0],x_length,y_length); // start the real time median mask for subband 1
                startParallelProcThread(1,1,dataBundle[4],dataBundle[1],x_length,y_length); // start the real time median mask for subband 2
                startParallelProcThread(1,1,dataBundle[5],dataBundle[2],x_length,y_length); // start the real time median mask for subband 3
            }
            else
            {
                startParallelProcThread(1,1,dataBundle[1],dataBundle[0],x_length,y_length);
            }
        }
    }
    ...
    {
         destFramePointIncrOnly = destFramePoint = framePoint + waveformNumber;
        ...
        {
            //MEDIANDEBUG qDebug()<<"Trigger Real Time Median - destFramePoint+1"<<destFramePoint+1;
...
void dataProcessor::postProcessingXCorRequested()
{
    if (isMemValid(dataBundle, dispBuffIndex, frameNum,__FUNCTION__) == false)
     ...
     int y_length = (buffScanHeight/buffScanInterval)+1;
